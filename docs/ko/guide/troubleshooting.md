...
거절했거나 터미널이 아닌 곳에서 실행했다면 승인한 뒤 같은 명령을 한 번 더 실행하세요. 다시 실행하면 필터가 켜집니다.
...
| 2 | 설정을 읽지 못함 |
| 3 | 설정을 저장하지 못함 |
| 4 | 120초 안에 승인되지 않음. 터미널에서 실행했다면 명령이 승인 화면을 열지 묻고 이어서 기다립니다. 그 밖에는 시스템 설정에서 승인하고 다시 실행하세요 |
| 5 | 맥을 재시작해야 완료됩니다 |
