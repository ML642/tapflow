...
**2분 안에 승인하지 못했다면 명령이 승인 화면을 열지 묻습니다.** 터미널에서 직접 실행한 경우에만 묻습니다. 동의하면 화면을 열고 tapflow 항목이 켜질 때까지 최대 2분 더 기다린 뒤 필터를 켭니다.
...
거절했거나 터미널이 아닌 곳에서 실행했거나 더 기다린 2분 안에 켜지 못했다면 명령은 승인 대기 상태로 끝납니다. 승인한 뒤 같은 명령을 한 번 더 실행하세요. 다시 실행하면 필터가 켜집니다.
...
| 2 | 설정을 읽지 못함 |
| 3 | 설정을 저장하지 못함 |
| 4 | 120초 안에 승인되지 않음. 터미널에서 실행했다면 명령이 승인 화면을 열지 묻고 이어서 기다립니다. 거기서도 켜지지 않았거나 터미널이 아니었다면 명령은 승인 대기로 끝나므로 시스템 설정에서 승인하고 다시 실행하세요 |
| 5 | 맥을 재시작해야 완료됩니다 |
