...
릴레이는 자기 머신에서 릴레이 포트로 들어오는 연결만 인증 없이 받고, 나머지 모든 연결에는 인증을 요구합니다. 이 섹션은 그중 에이전트 쪽을 다룹니다. 터널 클라이언트도 같은 머신에서 실행되므로, 인증이 반드시 필요한 별도의 터널 포트로 연결합니다. 브라우저가 사무실 밖에서 릴레이에 접근하는 방법(터널)은 [릴레이 배포의 외부 접속](/ko/guide/self-hosting#외부-접속)을 참고하세요.
