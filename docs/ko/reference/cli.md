...
macOS에서 `setup ios`는 iOS 네트워크 제어에 필요한 네트워크 필터도 설치하는데, 다른 설치와 마찬가지로 먼저 동의를 구합니다. macOS의 승인이 제때 오지 않으면 승인 화면을 열지 묻고 동의하면 켜질 때까지 기다렸다가 설치를 마칩니다. 설치를 거절했거나 그 기능이 나오기 전에 설정한 맥이라면 [`tapflow migrate net-filter`](#tapflow-migrate-net-filter)로 따로 설치합니다.
...
**승인이 늦어도 같은 실행에서 끝까지 갑니다.** 설치는 승인을 2분까지 기다립니다. 그 안에 승인되지 않았고 대화형 터미널에서 실행 중이면 승인 화면을 열지 묻습니다. 질문에는 필터를 켤 때 맥에 이미 열려 있던 연결이 끊길 수 있다는 내용도 들어 있습니다. SSH 세션도 여기 포함됩니다. 동의하면 화면을 열고 tapflow 항목이 켜질 때까지 다시 최대 2분 기다립니다. 그 안에 켜지면 필터를 켜고 실제로 도는지 확인합니다. 켤 때 macOS가 네트워크 콘텐츠 필터링을 허용할지 물으면 허용하세요.

**화면이 열렸다고 말하지 않습니다.** 창이 떴는지 명령이 알 방법이 없어서 경로를 함께 보여줍니다. 아무것도 뜨지 않으면 그 경로로 직접 가세요.

**켜기 직전에 기기를 다시 확인합니다.** 기다리는 동안 누군가 시뮬레이터를 부팅했을 수 있기 때문입니다. 찾으면 필터를 켜지 않고 멈추며 필터가 꺼진 상태라면 그것도 알려줍니다. `--ignore-running-devices`를 줬다면 다시 확인하지 않습니다.

대화형 터미널이 아니면 묻지 않습니다. stdin과 stdout이 모두 터미널이어야 대화형으로 봅니다. 이때와 제안을 거절했을 때, 그리고 더 기다린 2분 안에 켜지지 않았을 때는 승인 대기 상태로 끝나며 승인한 뒤 명령을 다시 실행하라고 안내합니다. 다시 실행하면 필터가 켜집니다.
