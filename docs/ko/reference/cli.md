...
**승인이 늦어도 같은 실행에서 끝까지 갑니다.** 설치는 승인을 2분까지 기다립니다. 그 안에 승인되지 않았고 대화형 터미널에서 실행 중이면 승인 화면을 열지 묻습니다. 질문에는 필터를 켤 때 맥에 이미 열려 있던 연결이 끊길 수 있다는 내용도 들어 있습니다. SSH 세션도 여기 포함됩니다. 동의하면 화면을 열고 tapflow 항목이 켜질 때까지 다시 최대 2분 기다린 뒤, 필터를 켜고 실제로 도는지 확인합니다. 켤 때 macOS가 네트워크 콘텐츠 필터링을 허용할지 물으면 허용하세요.
...
대화형 터미널이 아니면 묻지 않습니다. stdin과 stdout이 모두 터미널이어야 대화형으로 봅니다. 이때와 제안을 거절했을 때는 승인한 뒤 명령을 다시 실행하라고 안내합니다. 다시 실행하면 필터가 켜집니다.
