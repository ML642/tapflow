...
**Switching it off and removing it are different things.** System Settings → General → Login Items & Extensions → Network Extensions turns it off and leaves it installed. To remove it, switch the filter off first:

```sh
/Applications/TapflowNetFilter.app/Contents/MacOS/TapflowNetFilter --off
```

Then delete the extension from the ⋯ button beside TapflowNetFilter on that same screen, and restart the Mac. The removal finishes at the restart; until then `systemextensionsctl list` shows it as `terminated waiting to uninstall on reboot`. The filter goes off first because deleting one that is on can block the Mac's new connections.
