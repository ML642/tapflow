...
The offer is made when no tapflow extension is approved yet (a first install, a rerun over a waiting request, a reinstall after removing it), and only in an interactive terminal. It carries the same warning as before about connections dropping when the filter goes on. A yes is not asked for again after the wait, and a no is not repeated. When macOS was not expected to ask, the offer after the wait is unchanged. The activation line now tells people to choose Open System Settings rather than OK.
