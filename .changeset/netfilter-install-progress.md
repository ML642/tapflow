...
`tapflow setup ios` reports the first check too, on the path where it then *skips* the install. That is the common case — a Mac that is already set up — and it decides it from the same four probes, so it was the half of the silence a report inside the installer could never reach.
