...
- **The iOS helpers find SimulatorKit where Xcode 27 moved it** ([#797](https://github.com/jo-duchan/tapflow/issues/797)). Xcode 27 moved `SimulatorKit.framework` from `Contents/Developer/Library/PrivateFrameworks/` to `Contents/SharedFrameworks/`, and the touch and screen-capture helpers knew only the old place: with Xcode 27 alone no input reached the simulator and no stream opened, and with Xcode 26 installed beside it they quietly loaded 26's SimulatorKit instead. Both places are now checked on the selected Xcode first. Devices whose bezel is drawn from nine slices lost it too, because Xcode 27 moved the screen size from `profile.plist` to `capabilities.plist`; either file is now read.
